...
object equipmentRpt {
  def main(args: Array[String]): Unit = {
    ...
    val operators: RDD[(String, List[Double])] = df.map(row => {
      ...
      // 设备类型
      val devicetype: Integer = row.getAs[Integer]("devicetype")
      ...
      // 返回一个元组  形式:(设备类型,九个指标)
      (isType(devicetype), reqlist ++ clicklist ++ adlist)
    })
    ...
    }).map(x => (x._1, x._2(0), x._2(1), x._2(2), x._2(3), x._2(4),
      if (x._2(3) == 0) 0 else x._2(4) / x._2(3), x._2(5), x._2(6), if (x._2(5) == 0) 0 else x._2(6) / x._2(5),
      x._2(7), x._2(8)))
      .toDF("设备类型", "总请求", "有效请求", "广告请求", "参与竞价数", "竞价成功数",
        "竞价成功率", "展示量", "点击量", "点击率", "广告消费", "广告成本")
    res.show()
    res.write.jdbc(url, "devicetype", connectionProperties)

    sc.stop()
  }

  def isType(devicetype:Integer) :String ={
    if(devicetype == 1){
      "手机"
